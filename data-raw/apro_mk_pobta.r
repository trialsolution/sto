...
# load the old file... 
#load(file = paste(extraction_folder,"apro_mk_pobta_2024-07-02.RData",sep = ""))
load(file = paste("U:/4-Market Analysis/4-2 Short-Term Outlook/Outlook Dairy/Short term dairy/2024_2/Eurostat download with R/",
                  "apro_mk_pobta_2024-08-27.RData",sep = ""))
old <- apro_mk_pobta
rm(apro_mk_pobta)

# save the new apro_mk_pobta dataset on 'new'
load(file = paste(extraction_folder,"apro_mk_pobta_2025-06-05.RData",sep = ""))
new <- apro_mk_pobta
rm(apro_mk_pobta)
